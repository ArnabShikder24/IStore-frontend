...
export default function OrderCard({ order }) {
    ...
    const { order_id, product_id, quantity, subtotal, user_id } = order;
    const [loading, setLoading] = useState(true);
    const [show, setShow] = useState(false);
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/api/v1/product?product_id=${product_id}`);
    ...
    const { title, img, description, color, category } = product;
    return (
        <>
            {
                product &&
                        ...
                        <div class="px-5 pb-5">
                            ...
                            </div>
                            {
                                ...
                                <div className="my-3">
                                    ...
                                    <input className="w-24 h-10 rounded-lg" type="number" value={quantity} />
                                </div>
                            }
